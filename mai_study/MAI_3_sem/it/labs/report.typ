...
функция main():
  ...
  ОБЪЯВИТЬ БУФЕР[512]
  ВЫВЕСТИ ГЛАВНОЕ МЕНЮ С ОПЦИЯМИ: 1 - автомобильные данные, 2 - регистрационные данные, q - выход
  ПОКА ПОЛЬЗОВАТЕЛЬ НЕ ВВЕЛ q:
    СЧИТАТЬ ВЫБОР ПОЛЬЗОВАТЕЛЯ
    ЕСЛИ ВЫБОР РАВЕН '1' ИЛИ '2' ТО:
      ВЫВЕСТИ ЗАГОЛОВОК ВЫБРАННОГО РЕЖИМА
      ВЫЗВАТЬ write_data(БУФЕР, РАЗМЕР_БУФЕРА, ВЫБОР)
...
функция file_info(БУФЕР, РАЗМЕР_БУФЕРА, ИМЯ_ПО_УМОЛЧАНИЮ):
  ...
  ЕСЛИ ФАЙЛ СУЩЕСТВУЕТ ТО:
    ЗАПРОСИТЬ РЕЖИМ РАБОТЫ: добавить (Enter), перезаписать (N), отмена (q)
...
функция write_data(БУФЕР, РАЗМЕР_БУФЕРА, ТИП_ДАННЫХ):
  ВЫЗВАТЬ file_info ДЛЯ ПОЛУЧЕНИЯ ИМЕНИ И РЕЖИМА ОТКРЫТИЯ ФАЙЛА
  ...
  ВЫВЕСТИ ПОДСКАЗКИ: 'q' - завершить ввод, 'menu' - выход в меню
...
#table(
  ...
  [cstdlib],
  [стандартная библиотека C (EXIT_SUCCESS)],
...
#rect(width: 100%)[
  *Тест 9. Проверка выхода из программы*\
  \
  В главном меню пользователь вводит 'q'.\
  Программа выводит сообщение "Программа завершена." и завершает свою работу.\
  \
  *Проверка выхода через 'menu' во время ввода:*\
  При заполнении полей пользователь вводит 'menu'.\
  Программа выводит "Возврат в главное меню (текущая запись не сохранена)" и возвращается в главное меню без сохранения текущей записи.
]
