...
#table(
  ...
  [cstring],
  [библиотека для подключения функции memset],

  [string],
  [библиотека для работы с "умными" строками],
  ...
  [
    ...
    нужна для проверки существования файла
  ],
) 
...
#table(
  ...
  [CarData],
  [Структура для хранения автомобильных сведений],
  [
    *license* -- поле хранит информацию про номерной знак автомобиля,\
    *brand* -- поле хранит информацию про бренд автомобиля,\
    *model* -- поле хранит информацию про модель автомобиля
  ],

  [LicenseData],
  [Структура для хранения регистрационных сведений],
  [
    *license* -- поле хранит информацию про номерной знак автомобиля,\
    *surname* -- поле хранит информацию про фамилию владельца автомобиля,\
    *address* -- поле хранит информацию про адрес владельца автомобиля,\
    *release_year* -- поле хранит год выпуска автомобиля
  ],
) 
...
#table(
  ...
  [Введен %c программа продолжает работу\\n],
  [Введенный символ не является N,n,Q,q или "enter"("\\n").
],

  [Введен %c. Можно ввести только опцию 1 или опцию 2. Попробуйте еще раз."],
  [Введенный символ не является 1 или 2.],

  [Введена пустая строка],
  [Пользователь ввел enter("\\n") или что-то с нулевой длиной в поле год выпуска.
],

  [Строка не полностью состоит из цифр"],
  [Поле год, введенное пользователем, содержит что-то кроме цифр.
],

  [Год вне диапазона(%d-%d)"],
  [Поле год, введенное пользователем, выходит за описанные временные рамки.],

  [Введенное имя файла некорректно. Файл должен иметь вид file.txt, где file состоит из латинских символов, дефиса(-) или нижнего подчеркивания(```_```), а после следует его расширение .txt"],
  [Пользователь ввел неподходящее имя файла.],

  [Файл %s уже существует.\\nЖелаете продолжить запись вместо заполнения с начала(enter=Да/N=Нет)?],
  [Пользователь ввел имя файла, который уже существует.],
...
В результате тестирования можно отметить, что программа
